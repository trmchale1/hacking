...
```shell-session
trmchale@htb[/htb]$ gem install evil-winrm
```
...
```shell-session
trmchale@htb[/htb]$ evil-winrm -i 10.129.201.234 -u forend
...
```shell-session
trmchale@htb[/htb]$ mssqlclient.py 
...
```shell-session
trmchale@htb[/htb]$ mssqlclient.py INLANEFREIGHT/DAMUNDSEN@172.16.5.150 -windows-auth
