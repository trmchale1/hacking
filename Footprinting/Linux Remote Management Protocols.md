...
```shell-session
trmchale@htb[/htb]$ cat /etc/ssh/sshd_config  | grep -v "#" | sed -r '/^\s*$/d'
...
```shell-session
trmchale@htb[/htb]$ git clone https://github.com/jtesta/ssh-audit.git && cd ssh-audit
trmchale@htb[/htb]$ ./ssh-audit.py 10.129.14.132
...
```shell-session
trmchale@htb[/htb]$ ssh -v cry0l1t3@10.129.14.132
...
```shell-session
trmchale@htb[/htb]$ ssh -v cry0l1t3@10.129.14.132 -o PreferredAuthentications=password
...
```shell-session
trmchale@htb[/htb]$ sudo nmap -sV -p 873 127.0.0.1
...
```shell-session
trmchale@htb[/htb]$ nc -nv 127.0.0.1 873
...
```shell-session
trmchale@htb[/htb]$ rsync -av --list-only rsync://127.0.0.1/dev
...
```shell-session
trmchale@htb[/htb]$ cat /etc/hosts.equiv
...
```shell-session
trmchale@htb[/htb]$ sudo nmap -sV -p 512,513,514 10.0.17.2
...
```shell-session
trmchale@htb[/htb]$ rlogin 10.0.17.2 -l htb-student
...
```shell-session
trmchale@htb[/htb]$ rusers -al 10.0.17.5
...
 [Previous](https://academy.hackthebox.com/module/112/section/1245)

 Mark Complete & Next

[Next](https://academy.hackthebox.com/module/112/section/1242) 
