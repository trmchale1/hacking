...
```shell-session
trmchale@htb[/htb]$ cat /etc/snmp/snmpd.conf | grep -v "#" | sed -r '/^\s*$/d'
...
```shell-session
trmchale@htb[/htb]$ snmpwalk -v2c -c public 10.129.14.128
...
```shell-session
trmchale@htb[/htb]$ sudo apt install onesixtyone
trmchale@htb[/htb]$ onesixtyone -c /opt/useful/SecLists/Discovery/SNMP/snmp.txt 10.129.14.128
...
```shell-session
trmchale@htb[/htb]$ sudo apt install braa
trmchale@htb[/htb]$ braa <community string>@<IP>:.1.3.6.*   # Syntax
trmchale@htb[/htb]$ braa public@10.129.14.128:.1.3.6.*
